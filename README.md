...
This tap extracts report data from extracting data from the [Google Analytics Reporting API](https://developers.google.com/analytics/devguides/reporting/core/v4/) via default column & table names.
...
| [Partial Import](https://docs.y42.com/docs/features#partial-import)                     | ✅     |         |
| [Start Date Selection](https://docs.y42.com/docs/features#start-date-selection)         | ✅     |         |
| [Import Empty Tables](https://docs.y42.com/docs/features#import-empty-table)            | ❌     |         |
| [Custom Data](https://docs.y42.com/docs/features#custom-data)                           | ❌     |         |
| [Retroactive Updating](https://docs.y42.com/docs/features#retroactive-updating)         | ❌     |         |
| [Dynamic Column Selection](https://docs.y42.com/docs/features#dynamic-column-selection) | ✅     |         |      
...
- **revenue_sources**: Transactions and transactions revenue metrics on source, medium, social network and adcontent
- **goal_conversions**: Completion of goals on source, medium, campaign and country dimensions
- **goal_values**: Value of goals on source, medium, campaign and country dimensions
- **campaign_adcontent_performance**: Most common metrics per day in addition to transactions and adCost on various dimensions (source, medium, campaign, adcontent, keyword, referrer)
- **revenue_transactions**: Transaction revenue per day on source, medium, campaign, channel grouping, device category and transaction
- **page_flow_tracking**: Most common page metrics and transitions per day on page path and page depth
- **ecommerce_geo**: E-Commerce metrics (Transactions and product (cart, checkout) metrics) per day on geo (country, region, etc.) & user type
- **ecommerce_sources**: E-Commerce metrics per day on source, medium, page path, channel grouping and user type
- **ecommerce_report**: E-Commerce product metrics per day on user type
- **ecommerce_landing_report**: Session and purchase metrics per day on country and page path
- **coupon_report**: Revenue & transaction metrics per day on source, medium, campaign and coupon
...
In general, Analytics API has some quotas on failed requests:
...
Also, Analytics Reporting API v4 has some special quotas:
...
The Analytics Reporting API v4 uses the **pageToken** and **pageSize** fields to paginate through response results that span multiple pages.
If a request has more than one page, it generates **nextPageToken** attribute in response which can be used for getting next page.
...
2. Create the config file
   ...
   Create a JSON file called `config.json`. Its details can be found in "Configuration Settings" section.

4. (Optional) Run the Tap in Discovery Mode

   `tap-google-analytics -c config.json -d`
...
5. Run the Tap in Sync Mode
   ...
   `tap-google-analytics -c config.json [--catalog catalog.json]`
...
If you already have a valid `client_secrets.json` for a service account, or if you are using OAuth based authorization, you can skip the rest of this section.
